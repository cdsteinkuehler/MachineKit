...
#include "hal.h"
#include <stdio.h>
// FIXME change to use hm2_Xi90.h
...
//fprintf(stderr,"%s.gpio.%03d.in\n",hm2->llio->name,i);
        ...
        // pins
        r = hal_pin_bit_newf(HAL_OUT,&(hm2->pin[i].instance->hal.pin.in),hm2->llio->comp_id,
                             "%s.gpio.%03d.in",
                             hm2->llio->name,i
                            );
        if (r < 0) {
            //HM2_ERR(
            fprintf(stderr,
            "error %d adding gpio pin, %s.gpio.%03d.in aborting\n", r,hm2->llio->name,i);
            return -EINVAL;
        }

        r = hal_pin_bit_newf(HAL_OUT,&(hm2->pin[i].instance->hal.pin.in_not),hm2->llio->comp_id,
                             "%s.gpio.%03d.in_not",
                             hm2->llio->name, i
                            );
        if (r < 0) {
            //HM2_ERR(
            fprintf(stderr,
            "error %d adding gpio pin, aborting\n", r);
            return -EINVAL;
        }
        ...
        if (   (hm2->pin[i].gtag == HM2_GTAG_IOPORT)
           ...
           ) 
          {
            ...
            r = hal_param_bit_newf(HAL_RW,&(hm2->pin[i].instance->hal.param.invert_output),hm2->llio->comp_id,
            ...
            if (r < 0) {
                //HM2_ERR(
                fprintf(stderr,
                 "error %d adding gpio param, aborting\n", r);
                return -EINVAL;
            }

            r = hal_param_bit_newf(HAL_RW,&(hm2->pin[i].instance->hal.param.is_opendrain),hm2->llio->comp_id,
        ...
        if (hm2->pin[i].gtag == HM2_GTAG_IOPORT) {
            ...
            r = hal_pin_bit_newf(HAL_IN,&(hm2->pin[i].instance->hal.pin.out),hm2->llio->comp_id,
                                 "%s.gpio.%03d.out",
                                 hm2->llio->name,i
                                );
            if (r < 0) {
            ...
            *(hm2->pin[i].instance->hal.pin.out) = 0;
            ...
            // parameters
            r = hal_param_bit_newf(HAL_RW,&(hm2->pin[i].instance->hal.param.is_output),hm2->llio->comp_id,
