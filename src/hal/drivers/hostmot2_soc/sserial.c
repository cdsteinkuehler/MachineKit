...
int hm2_sserial_parse_md(hostmot2_t *hm2, int md_index){
    ...
    //
fprintf(stderr,"SmartSerial Parse\n");
    ...
    //
fprintf(stderr,"SmartSerial Initialize\n");
             ...
             if (hm2->pin[pin].sec_tag == HM2_GTAG_SMARTSERIAL)
                {
                 ...
                 //HM2_DBG(
                 fprintf(stderr,  
                          "sec unit = %i, sec pin = %i\n", hm2->pin[pin].sec_unit, hm2->pin[pin].sec_pin);
                     src_reg |= (1 << port_pin);
                     if (hm2->pin[pin].sec_pin & 0x80)
                        {ddr_reg |= (1 << port_pin);
                        }
                    }
                }
            }// end of for(port_pin
fprintf(stderr,"Port %d ddr = %08X\n",port,ddr_reg);
fprintf(stderr,"Port %d src = %08X\n",port,src_reg);
...
fprintf(stderr,"now try to write to those ports\n");
...
fprintf(stderr,"Reset %08x\n",inst->command_reg_addr);
        buff=0x0001; //Clear
        HM2WRITE(inst->command_reg_addr, buff);
fprintf(stderr,"clear %08x\n",inst->command_reg_addr);
...
//-------        
        for (c = 0 ; c < inst->num_channels ; c++) 
             ...
             if (hm2->sserial.baudrate == 115200
                 && hm2->config.sserial_modes[i][c] != 'x')  //setup mode
                {
                 ...
                 if ((user1 & 0xFF00) == 0x4900)
                    ...
                    { // Look for 8i20s with the CRC check off
                     ...
                     || user1 == HM2_SSERIAL_TYPE_7I69  //#define HM2_SSERIAL_TYPE_7I69   0x39364937
                    )
                   {fprintf(stderr,"USER2 %d\n",user2);
                    inst->num_remotes += 1;
                    inst->tag |= 1<<c;
                   } 
            ...
            // make the pins into GPIO. 
                ...
                //HM2_ERR(
                fprintf(stderr,
                        "Unsupported Device (%4s) found on sserial %d "
                        "channel %d\n", (char*)&user1, i, c);
            }
        }
//--------
...
fail0:
fprintf(stderr,"SmartSerial FAIL\n");
