...
//    u32 instance_stride, // the hard coded 0x10 (16 decimal) below will contradict what the rom has
// FIXME temp change to 64 decimal 0x40
//    if (!hm2_md_is_consistent_or_complain(hm2, md_index, 0, 4, 0x10, 0x000F)) {
    if (!hm2_md_is_consistent_or_complain(hm2, md_index, 0, 4, 0x40, 0x000F)) {
...
// FIXME
fprintf(stderr," UART NUM INSTANCES %d STRIDE %d\n", hm2->uart.num_instances,md->instance_stride);   
...
//FIXME added line below as it didn't seem to be called
    hm2_uart_print_module(hm2);
