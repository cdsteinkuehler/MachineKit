...
//direction.
    switch (pin->sec_tag) {
        ...
        case HM2_GTAG_XIOPORT:
             switch (sec_pin) {
                case 0x01: return "PCS";
                case 0x02: return "PENA";
                case 0x03: return "PDIR";
                case 0x04: return "PADDR0";
                case 0x05: return "PADDR1";
                case 0x06: return "PADDR2";
                case 0x07: return "PDATA0";
                case 0x08: return "PDATA1";
                case 0x09: return "PDATA2";
                case 0x0A: return "PDATA3";
                case 0x0B: return "PDATA4";
                case 0x0C: return "PDATA5";
                case 0x0D: return "PDATA6";
                case 0x0E: return "PDATA7";
            }
             break;
...
static void hm2_print_pin_descriptors(hostmot2_t *hm2) {
    ...
    for (i = 0; i < hm2->num_pins; i ++) {
        ...
        //HM2_PRINT(
        fprintf(stderr,    "        Primary Tag: 0x%02X (%s)\n",
            hm2->pin[i].primary_tag,
            hm2_get_general_function_name(hm2->pin[i].primary_tag)
        );
        if (hm2->pin[i].sec_tag != 0) {
        //    HM2_PRINT(
            fprintf(stderr,"        Secondary Tag: 0x%02X (%s)\n",
                hm2->pin[i].sec_tag,
                hm2_get_general_function_name(hm2->pin[i].sec_tag)
            );
        //    HM2_PRINT(
            fprintf(stderr,    "        Secondary Unit: 0x%02X\n", hm2->pin[i].sec_unit);
        //    HM2_PRINT(
            fprintf(stderr,    "        Secondary Pin: 0x%02X (%s, %s)\n",
                hm2->pin[i].sec_pin,
...
void hm2_print_pin_usage(hostmot2_t *hm2) {
    ...
    //HM2_PRINT(
     fprintf(stderr,"%d I/O Pins used:\n", hm2->num_pins);
...
void hm2_configure_pins(hostmot2_t *hm2) {
    ...
    hm2_pins_allocate_all(hm2, HM2_GTAG_XIOPORT,  hm2->xioport.num_instances);
}
